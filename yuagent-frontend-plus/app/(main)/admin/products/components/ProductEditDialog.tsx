...
import { Product, ProductFormData, UpdateProductRequest } from "@/types/product";
...
export function ProductEditDialog({ open, onOpenChange, product, onSuccess }: ProductEditDialogProps) {
  ...
  useEffect(() => {
    if (open && product) {
      setFormData({
        name: product.name,
        type: product.type as BillingType,
      ...
      loadRules();
      loadBusinessOptions(product.type);
    }
  }, [open, product]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await AdminProductServiceWithToast.updateProduct(product.id, request);
