...
export default function AdminUploadToolPage() {
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      // 调用管理员上传工具API
      const response = await AdminToolService.uploadOfficialTool(payload);
      if (response.code === 200) {
        ...
        toast({
          title: "创建失败",
          description: response.message || "创建工具时出现错误",
          variant: "destructive"
        });
      }
    } catch (error) {
    ...
    setTimeout(() => {
      ...
      if (selectedText) {
        ...
        if (selectionOffsets) {
          ...
          const placeholderEnd = placeholderStart + 2;
  ...
  return (
    <div className="container py-6">
      <div className="mb-6">
        ...
        <h1 className="text-3xl font-bold tracking-tight">创建官方工具</h1>
        <p className="text-muted-foreground">创建并发布官方工具到工具市场</p>
      </div>
